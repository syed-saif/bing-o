document.addEventListener("DOMContentLoaded", function() {
    ...
    socket.emit('joined second game page', {
        'username': username,
        'room_id': room_id
    });
    ...
    function update_current_turn(user) {
        if (user == username) {
            ...
            document.getElementById('ct').innerHTML = 'Current turn: ' + user;
        }
    }
    ...
    function left() {
        socket.emit('left second game page', {
            'username': username,
            'room_id': room_id,
            'current_turn': isTheirCurrentTurn
        });
    }
            ...
            if (data.hasOwnProperty(user)) {
                ...
                if (scores[user] == 5) { //as soon as one player finishes their name is pushed to 'positions' array in all the clients
                    ...
                    if (user == username) { //only for that client , a socket message is emitted(to save network calls) and finished prompt is displayed
                        ...
                        const p = document.createElement('p');
                        p.innerHTML = 'You have finished! Waiting for others to finish...';
